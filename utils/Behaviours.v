...
From Utils    Require Import Smallstep.
...
Hint Unfold behaviour_matches behaviour_improves : semMetaThy.
...
(* Theorem program_behaves_unique_Terminates:
  (exists v, program_behaves (Terminates v)) ->
  (forall e, ~ program_behaves (Faultstops e)) /\
  ~ program_behaves Diverges /\
  ~ program_behaves Goes_wrong.
Proof with auto.
  intros [beh ?].
...
(* Definition safe_along_behavior (s: state L1) (b: program_behaviour) : Prop :=
  forall s', Star L1 s s' ->
     (exists r, final_state L1 s' r)
  \/ (exists s'', Step L1 s' s'').
...
(* Remark star_safe_along:
  ...
  safe_along_behavior s' b.
Proof with eauto.
  red. intros. eapply H. eapply star_trans...
Qed. *)
...
(*       exploit backward_simulation_star; eauto.
      ...
      * intros [s1'' [res1 [D [FIN MV]]]].
        ...
        | ]...
    + (* fault stop *)
(*       exploit backward_simulation_star...
      intros [i' [s1' [A B]]]. *)
