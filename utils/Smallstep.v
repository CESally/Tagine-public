...
(* Arguments Backward_simulation L1 L2 match_values match_errors index order match_states props. *)
...
(* this relation seems to be talking about the excess/admin steps 
   needed to reach a state that matches the beginning of a simulated
   sequence in the target *)

(* [TOL] This might be talking about progress, and is saying
   (in the final and error cases) that we can't really make
    progress, so it's sort of progress as good as you can get.
...
(* (* only makes sense for traces *)
Lemma f2b_determinacy_star:
  forall s s1, Star L2 s s1 ->
  forall s2 s3,
  Step L2 s1 s2 ->
  Star L2 s s3 ->
  Star L2 s1 s3.
Proof with eauto.
  intros s0 s01 ST0. pattern s0, s01.
  eapply star_ind...
  intros. inv H3.
  - admit.
  - erewrite <- (f2b_determinacy_inv _ _ _ H H4) in *...
Admitted. *)
...
Inductive f2b_match_states: f2b_index -> state L1 -> state L2 -> Prop :=
  ...
  | f2b_match_before: forall s1 s1' s2b s2 n s2a i,
      Step L1 s1 s1' ->
      Star L2 s2b s2 ->
